...
class TestRobot:
    def test_move_alters_position_correctly(
        ...
        assert placed_robot_north_facing.position == Point(
    ...
    def test_clockwise_circle_returns_to_origin(
        ...
        for command in commands_to_return_to_starting_point:
            command()
            if path[-1] != robot.position and robot.position is not None:
    ...
    def test_next_position_returns_candidate_without_mutating(
        ...
        starting_position = placed_robot_north_facing.position
        assert starting_position is not None
        assert candidate == Point(starting_position.x, starting_position.y + 1)
        assert placed_robot_north_facing.position == starting_position
